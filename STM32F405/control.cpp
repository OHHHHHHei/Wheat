...
void CONTROL::Control_AutoAim()//自瞄控制函数
{
	...
	if (xuc.RxNuc_TJ.mode_TJ != 0)  //表示是否检测到目标
	{
		...
		pantile.mark_yaw = yaw_angle_mech + yaw_vel_feedforward + yaw_acc_feedforward;
		pantile.mark_pitch = pitch_angle_mech + pitch_vel_feedforward + pitch_acc_feedforward;
